...
class GoogLeNet(nn.Module):
    def __init__(self, block=Inception, filters=None, dataset=None,compress_rate=None):
        ...
        else : self.linear = nn.Linear(sum(filters[-1]), 100)
...
def googlenet(dataset,compress_rate=None):
    return GoogLeNet(block=Inception, dataset=dataset,compress_rate=compress_rate)
