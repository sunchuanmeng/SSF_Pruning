...
class ResNet(nn.Module):
    def __init__(self, block, num_layers, covcfg, dataset,compress_rate, num_classes=10):
...
def resnet_56(dataset,compress_rate=None):
    cov_cfg = [(3 * i + 2) for i in range(9 * 3 * 2 + 1)]
    return ResNet(ResBasicBlock, 56, cov_cfg,dataset, compress_rate=compress_rate)
...
def resnet_110(dataset,compress_rate=None):
    cov_cfg = [(3 * i + 2) for i in range(18 * 3 * 2 + 1)]
    return ResNet(ResBasicBlock, 110, cov_cfg,dataset, compress_rate=compress_rate)
